...
const Sidebar = () => {
  return (
    <aside className="z-sidebar flex h-full min-w-sidebar w-sidebar flex-col border-r border-border-panel bg-surface-panel max-desktop:w-full max-desktop:min-w-0 max-desktop:border-r-0 max-desktop:border-b">
      ...
      <section className="border-b border-border-section px-4 py-section-y">
        ...
        <div className="mt-5 flex h-field-h overflow-hidden rounded border border-border-input bg-white">
          <input type="text" placeholder="위치 검색" className="flex-1 border-0 bg-transparent px-3 text-sidebar-input outline-none" />
          <button
            type="button"
            aria-label="위치 찾기"
        ...
        <div className="mt-5 flex flex-col gap-group-gap">
          {toggleOptions.map((toggle) => (
            <div key={toggle.key} className="flex items-center justify-between text-sidebar-body text-text-primary">
              ...
              >
                <span className={`block h-6 w-6 rounded-full bg-white transition ${toggle.enabled ? "translate-x-section-y" : ""}`} />
              </button>
            </div>
          ))}
        </div>
